...
std::string CoverRawCache::getRawPath(const std::string& bmpPath, int width, int height) {
  ...
  char filename[96];
  std::snprintf(filename, sizeof(filename), "%08lx_%dx%d.raw", static_cast<unsigned long>(h), width, height);
...
uint8_t* CoverRawCache::load(const std::string& rawPath, RawCoverHeader* outHeader, size_t* outSize) {
  ...
  if (header.width == 0 || header.height == 0 || outputRowSize == 0 || header.rowBytes != outputRowSize) {
    file.close();
    return nullptr;
  }
