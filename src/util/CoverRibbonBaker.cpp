...
void drawReadRibbon(uint8_t* buf, int rowBytes, int w, int h) {
  ...
  for (int i = 0; i < 4; ++i) {
    setPixel(buf, rowBytes, cx - 7 + i, cy + i, false);
  }
  for (int i = 0; i < 5; ++i) {
    setPixel(buf, rowBytes, cx - 3 + i, cy + 4 - i, false);
  }
}
...
void drawFavoriteRibbon(uint8_t* buf, int rowBytes, int w, int h) {
  ...
  for (int dy = 2; dy <= 4; ++dy)
    for (int dx = 0; dx < 11; ++dx) setPixel(buf, rowBytes, hx + dx, hy + dy, false);
  // Row 6:  _ x x x x x x x x x _
  for (int dx = 1; dx < 10; ++dx) setPixel(buf, rowBytes, hx + dx, hy + 6, false);
  // Row 8:  _ _ _ x x x x x _ _ _
  for (int dx = 3; dx < 8; ++dx) setPixel(buf, rowBytes, hx + dx, hy + 8, false);
  // Row 9:  _ _ _ _ x x x _ _ _ _
  for (int dx = 4; dx < 7; ++dx) setPixel(buf, rowBytes, hx + dx, hy + 9, false);
}
    ...
    for (int dy = -dotR; dy <= dotR; ++dy) {
      for (int dx = -dotR; dx <= dotR; ++dx) {
        if (dx * dx + dy * dy <= dotR * dotR + dotR) {
          int px = dotCX + dx;
          int py = dotCY + dy;
          if (px >= 0 && px < w && py >= 0 && py < h) setPixel(buf, rowBytes, px, py, false);
        }
      }
    }
  }
...
void bakeIntoFile(const std::string& coverPath, const std::string& bookPath) {
  ...
  if (f.seek(10)) {
    uint8_t ob[4];
    if (f.read(ob, 4) == 4)
      bfOffBits = ob[0] | (static_cast<uint32_t>(ob[1]) << 8) | (static_cast<uint32_t>(ob[2]) << 16) |
  ...
  if (pixelBytes > 256000) {
    f.close();
    return;
  }
  std::vector<uint8_t> pixels(pixelBytes);
  // Seek to pixel data
  if (!f.seek(static_cast<int>(bfOffBits))) {
    f.close();
    return;
  }
  if (f.read(pixels.data(), pixelBytes) < pixelBytes) {
    f.close();
    return;
  }
  f.close();
