...
class LibraryNavigation {
 public:
  ...
  static bool handleBack(const LibraryNavState& state, bool launchFromApps, LibraryNavActionResult* outAction);
  ...
  static bool handleConfirm(const LibraryNavState& state, const LibraryIndex::BookRef& pageRef,
                            LibraryNavActionResult* outAction);

  // Check if the current selection represents a root-level collection/series tile.
  static bool isRootCollectionTile(const LibraryNavState& state, const LibraryIndex::BookRef& pageRef);
};
