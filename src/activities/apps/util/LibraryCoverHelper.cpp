...
void LibraryCoverHelper::deletePageCovers(int gridsPerPage, const LibraryIndex::BookRef* pageCache,
...
bool LibraryCoverHelper::writeTextFallbackCover(GfxRenderer& renderer, const std::string& path,
                                                 int coverWidth, int coverHeight) {
...
bool LibraryCoverHelper::generatePageCover(GfxRenderer& renderer, const std::string& path,
                                           int coverWidth, int coverHeight) {
  const std::string thumbPath = LibraryIndex::thumbPathFor(path, coverWidth, coverHeight);
  if (thumbPath.empty()) return false;
  ...
  if (FsHelpers::hasEpubExtension(path)) {
    ...
    const bool ok = epub.generateAdaptiveThumbBmp(coverWidth, coverHeight);
    if (ok) {
