...
bool isSeriesTile(const LibraryIndex::BookRef& ref) { return (ref.id & 0x80000000u) != 0; }
...
bool LibraryNavigation::handleConfirm(const LibraryNavState& state, const LibraryIndex::BookRef& pageRef,
                                      LibraryNavActionResult* outAction) {
  if (!outAction) return false;
  outAction->handled = false;
...
bool LibraryNavigation::isRootCollectionTile(const LibraryNavState& state, const LibraryIndex::BookRef& ref) {
