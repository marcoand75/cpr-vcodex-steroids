...
void BatchCoverGenerationActivity::loop() {
  if (scanning_) {
    ...
    for (int p = 0; p < kPagesPerLoop && scanNextPage_ * kPageSize < total; ++p, ++scanNextPage_) {
      const int count = LibraryIndex::queryPage(page, scanNextPage_, kPageSize, LibraryIndex::SortMode::TITLE_ASC);
...
void BatchCoverGenerationActivity::render(RenderLock&&) {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, metrics.topPadding + 20, tr(STR_BATCH_GENERATE_COVERS), true,
                            EpdFontFamily::BOLD);
