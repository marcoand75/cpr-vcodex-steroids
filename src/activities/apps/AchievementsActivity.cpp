...
void AchievementsActivity::render(RenderLock&&) {
    ...
    const int rowWidth = pageWidth - sidePadding * 2 - scrollBarWidth - scrollBarGap;
    ...
    if (visibleIndexes.empty()) {
        ...
        // Calculate total height of the list (all items have same height)
        const int itemHeight = ROW_HEIGHT;
        const int totalHeight = static_cast<int>(visibleIndexes.size()) * itemHeight;

        int firstVisibleIndex = 0;
        const int maxVisibleRows = std::max(1, viewportHeight / itemHeight);
        ...
        for (int visibleIndex = firstVisibleIndex; visibleIndex < static_cast<int>(visibleIndexes.size()); ++visibleIndex) {
            ...
            const Rect rowRect{sidePadding, currentY, rowWidth, itemHeight - 4};
            if (selected) {
            ...
            const int textWidth = rowWidth - (textX - rowRect.x) - progressWidth - 18;
        ...
        if (totalHeight > viewportHeight) {
            ...
            const int scrollOffset = firstVisibleIndex * itemHeight;
