...
namespace {
// Steroids fork-only port shim: this branch releases font heap through
// FontCacheManager rather than a global freeFontMemory()/restoreFontMemory()
// pair, so keep the screensaver's calls funneled through one local helper.
void releaseFontsForScreensaver(const GfxRenderer& renderer) {
...
void ScreenSaverActivity::onEnter() {
  ...
  }
  {
    ...
    if (Storage.openFileForWrite("SS", callerFrameBufferPath_, f)) {
      const uint8_t* buf = renderer.getFrameBuffer();
  ...
  if (Storage.exists(callerFrameBufferPath_.c_str())) {
    bool restored = false;
    {
      ...
      if (Storage.openFileForRead("SS", callerFrameBufferPath_, f)) {
        const uint32_t bufSize = renderer.getBufferSize();
        uint8_t* target = const_cast<uint8_t*>(renderer.getFrameBuffer());
        if (bufSize > 0 && target) {
