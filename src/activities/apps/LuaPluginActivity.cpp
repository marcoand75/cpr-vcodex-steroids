...
LuaPluginActivity::LuaPluginActivity(const std::string& pluginName, GfxRenderer& renderer, MappedInputManager& input,
                                     bool launchFromApps, bool returnToPluginBrowser, bool launchInProcess)
...
void LuaPluginActivity::onEnter() {
  ...
  if (fileSize > lua_plugin::MAX_LUA_SOURCE_SIZE) {
    LOG_ERR(TAG, "Plugin too large: %u bytes (max %u)", (unsigned)fileSize, (unsigned)lua_plugin::MAX_LUA_SOURCE_SIZE);
    renderer.clearScreen(0xFF);
  ...
  LOG_INF(TAG, "Plugin '%s' running (allocated=%u bytes)", pluginName_.c_str(), lua_plugin::getAllocatedBytes());
...
void LuaPluginActivity::loop() {
  ...
  if (lua_plugin::vmHasFunction("onKey")) {
    if (!lua_plugin::vmCallCallback("onKey", 0)) {
      ...
      LOG_ERR(TAG, "onKey() crashed — exiting plugin '%s': %s", pluginName_.c_str(), lua_plugin::getLastError());
      showErrorScreen("onKey() error:", lua_plugin::getLastError());
      finish();
      return;
    }
...
void LuaPluginActivity::onExit() {
  ...
  LOG_INF(TAG, "Plugin exited. Free heap: %u, MaxAlloc: %u", lua_plugin::getFreeHeap(), lua_plugin::getMaxAlloc());
