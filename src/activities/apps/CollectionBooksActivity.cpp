...
static void s_onBack(void* ctx) { static_cast<CollectionBooksActivity*>(ctx)->finish(); }
...
static void s_onNavContinuous(void* ctx, int delta) {
  ...
  if (delta > 0) {
    self->selectedIndex_ =
        ButtonNavigator::nextPageIndex(self->selectedIndex_, static_cast<int>(self->books_.size()), self->pageItems_);
  } else if (delta < 0) {
    self->selectedIndex_ = ButtonNavigator::previousPageIndex(self->selectedIndex_,
                                                              static_cast<int>(self->books_.size()), self->pageItems_);
  }
  self->requestUpdate();
}
...
void CollectionBooksActivity::onExit() { Activity::onExit(); }

void CollectionBooksActivity::loop() { listInputMapper_.loop(mappedInput); }
...
void CollectionBooksActivity::render(RenderLock&&) {
  ...
  ListRenderHelper::drawList(
      renderer, layout, static_cast<int>(books_.size()), selectedIndex_,
