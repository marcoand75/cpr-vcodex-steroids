...
void PluginBrowserActivity::parsePluginHeaders(PluginEntry& entry) {
  ...
  while (*p && lineNum < 50) {
    ...
    if (strncmp(trimmed, "-- NAME:", 8) == 0) {
      ...
      while (entry.name.size() > 0 && (entry.name.back() == ' ' || entry.name.back() == '\t' ||
                                       entry.name.back() == '\r' || entry.name.back() == '\n')) {
        entry.name.pop_back();
      }
    } else if (strncmp(trimmed, "-- DESC:", 8) == 0) {
      entry.description = trimmed + 8;
      while (entry.description.size() > 0 && (entry.description.back() == ' ' || entry.description.back() == '\t' ||
      ...
      while (entry.icon.size() > 0 && (entry.icon.back() == ' ' || entry.icon.back() == '\t' ||
    ...
    if (mappedInput.wasPressed(MappedInputManager::Button::Confirm)) {
      if (menuIndex_ == 0) {
        ...
        if (togglePluginReboot(plugin)) {
          LOG_INF(TAG, "Plugin reboot toggled: %s -> %s", plugin.name.c_str(), plugin.fastReboot ? "RST" : "LIVE");
        }
      } else if (menuIndex_ == 1) {
        const PluginEntry& plugin = pluginList_[selectedIndex_];
        startActivityForResult(
            std::make_unique<ConfirmationActivity>(renderer, mappedInput, tr(STR_PLUGIN_OPTION_DELETE), plugin.name),
...
void PluginBrowserActivity::render(RenderLock&&) {
  ...
  renderer.drawText(UI_12_FONT_ID, 20 + iconSize + 10, iconY + (iconSize - lh12) / 2, tr(STR_PLUGINS), true,
                    EpdFontFamily::BOLD);
  ...
  if (pluginList_.empty()) {
    ...
    for (int i = first; i <= last; i++) {
      ...
      if (plugin.fastReboot) {
        ...
        renderer.drawText(UI_10_FONT_ID, badgeX + 2, textY, "RST", sel ? true : false, EpdFontFamily::BOLD);
      } else {
