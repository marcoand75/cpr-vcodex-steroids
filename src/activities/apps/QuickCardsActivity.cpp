...
std::string QuickCardsActivity::convertJpegToBmp(const std::string& sourcePath) {
    ...
    HalFile jpegFile = Storage.open(sourcePath.c_str());
    if (!jpegFile) return {};

    HalFile outFile = Storage.open(cachePath.c_str(), O_WRONLY | O_CREAT | O_TRUNC);
    if (!outFile) {
        jpegFile.close();
        return {};
    }
    ...
    bool ok = JpegToBmpConverter::jpegFileTo1BitBmpStreamWithSize(jpegFile, outFile, sw, sh, &permanentFailure);
