...
static void s_onBack(void* ctx) { static_cast<CollectionManageActivity*>(ctx)->finish(); }
...
static void s_onNavRelease(void* ctx, int delta) {
  ...
  if (self->viewMode_ == CollectionManageActivity::ViewMode::Collections) {
    ...
    if (delta > 0) {
      self->selectedIndex_ =
          ButtonNavigator::nextIndex(self->selectedIndex_, static_cast<int>(self->collections_.size()));
    } else if (delta < 0) {
      self->selectedIndex_ =
          ButtonNavigator::previousIndex(self->selectedIndex_, static_cast<int>(self->collections_.size()));
    }
    self->requestUpdate();
    return;
  }
  ...
  if (self->viewMode_ == CollectionManageActivity::ViewMode::Members) {
    ...
    if (delta > 0) {
      self->selectedMemberIndex_ =
          ButtonNavigator::nextIndex(self->selectedMemberIndex_, static_cast<int>(self->members_.size()));
    } else if (delta < 0) {
      self->selectedMemberIndex_ =
          ButtonNavigator::previousIndex(self->selectedMemberIndex_, static_cast<int>(self->members_.size()));
    }
    self->requestUpdate();
    return;
  }
...
static void s_onNavContinuous(void* ctx, int delta) {
  ...
  if (self->viewMode_ == CollectionManageActivity::ViewMode::Collections) {
    if (self->collections_.empty()) return;
    if (delta > 0) {
      self->selectedIndex_ = ButtonNavigator::nextPageIndex(
          self->selectedIndex_, static_cast<int>(self->collections_.size()), self->pageItems_);
    } else if (delta < 0) {
      self->selectedIndex_ = ButtonNavigator::previousPageIndex(
          self->selectedIndex_, static_cast<int>(self->collections_.size()), self->pageItems_);
    }
    self->requestUpdate();
    return;
  ...
  if (self->viewMode_ == CollectionManageActivity::ViewMode::Members) {
    if (self->members_.empty()) return;
    if (delta > 0) {
      self->selectedMemberIndex_ = ButtonNavigator::nextPageIndex(
          self->selectedMemberIndex_, static_cast<int>(self->members_.size()), self->pageItems_);
    } else if (delta < 0) {
      self->selectedMemberIndex_ = ButtonNavigator::previousPageIndex(
          self->selectedMemberIndex_, static_cast<int>(self->members_.size()), self->pageItems_);
    }
    self->requestUpdate();
    return;
...
void CollectionManageActivity::onExit() { Activity::onExit(); }
...
void CollectionManageActivity::render(RenderLock&&) {
  ...
  if (viewMode_ == ViewMode::Collections) {
    ...
    if (collections_.empty()) {
      ...
      ListRenderHelper::drawHints(renderer, mappedInput, tr(STR_BACK), nullptr, tr(STR_COLLECTION_CREATE), nullptr);
    ...
    ListRenderHelper::drawList(
        renderer, layout, static_cast<int>(collections_.size()), selectedIndex_,
        ...
        true);
    ListRenderHelper::drawHints(renderer, mappedInput, tr(STR_BACK), tr(STR_COLLECTION_RENAME),
                                tr(STR_COLLECTION_CREATE), tr(STR_COLLECTION_DELETE));
    renderer.displayBuffer();
    return;
  }
  ...
  if (viewMode_ == ViewMode::Members) {
    ...
    if (members_.empty()) {
      GUI.drawPopup(renderer, tr(STR_COLLECTION_EMPTY));
      ListRenderHelper::drawHints(renderer, mappedInput, tr(STR_COLLECTION_BACK_TO_LIST), tr(STR_COLLECTION_RENAME),
                                  tr(STR_COLLECTION_ADD_BOOK), tr(STR_COLLECTION_REMOVE_BOOK));
      renderer.displayBuffer();
      return;
    }
    ...
    const auto layout = ListLayout::compute(renderer);
    ListRenderHelper::drawList(
        renderer, layout, static_cast<int>(members_.size()), selectedMemberIndex_,
        [this](int index) { return members_[index].title; }, nullptr, nullptr, nullptr, true);
    ListRenderHelper::drawHints(renderer, mappedInput, tr(STR_COLLECTION_BACK_TO_LIST), tr(STR_COLLECTION_RENAME),
                                tr(STR_COLLECTION_ADD_BOOK), tr(STR_COLLECTION_REMOVE_BOOK));
    renderer.displayBuffer();
    return;
...
void CollectionManageActivity::addBookToCurrentCollection() {
  if (currentCollectionId_.empty()) return;
  startActivityForResult(std::make_unique<CollectionPickerActivity>(renderer, mappedInput, 0),
                         [this](const ActivityResult& result) {
                           if (result.isCancelled) {
                             requestUpdate();
                             return;
