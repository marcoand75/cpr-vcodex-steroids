...
void ButtonActionSelectorActivity::loop() {
  ...
  buttonNavigator.onNextRelease([this, itemCount] {
    if (selectedIndex < itemCount - 1) {
      selectedIndex++;
      if (selectedIndex >= startIndex + PanelDrawHelper::kMaxVisibleRows) {
        startIndex = selectedIndex - PanelDrawHelper::kMaxVisibleRows + 1;
      }
      requestUpdate();
    }
  });
...
void ButtonActionSelectorActivity::render(RenderLock&&) {
  ...
  // Title — use the current setting's label as context
  if (mode == Mode::SHORT_PWRBTN) {
    const int safeIdx = std::min<int>(static_cast<int>(currentValue), itemCount - 1);
    ...
    PanelDrawHelper::drawTitle(renderer, layout, I18N.get(actionToLabelId(
        static_cast<CrossPointSettings::BUTTON_ACTION>(currentValue))));
  }

  PanelDrawHelper::drawSeparator(renderer, layout);

  // Scrolling: keep selection centered when possible
