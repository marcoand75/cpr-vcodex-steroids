...
std::vector<BookContextMenuActivity::MenuItem> BookContextMenuActivity::buildMenuItems(
    const bool isFavorite, const bool isCompleted, const bool isEpubFormat, const bool isLibraryMode,
    const bool isHidden, const bool isInUserCollection) {
  ...
  if (isLibraryMode) {
    items.push_back(
        {MenuAction::HIDE_BOOK, isHidden ? StrId::STR_UNHIDE_BOOK : StrId::STR_HIDE_BOOK, LibraryIcon, 32, 32});
...
void BookContextMenuActivity::onEnter() {
  ...
  listInputMapper.setBackHandler(
      [](void* ctx) {
  ...
  listInputMapper.setConfirmHandler(
      [](void* ctx) {
        auto* self = static_cast<BookContextMenuActivity*>(ctx);
  ...
  auto onNav = [](void* ctx, int delta) {
    ...
    if (delta > 0) {
      ...
      self->selectedIndex =
          ButtonNavigator::previousIndex(self->selectedIndex, static_cast<int>(self->menuItems.size()));
    }
    self->requestUpdate();
  };
...
void BookContextMenuActivity::loop() { listInputMapper.loop(mappedInput); }
...
void BookContextMenuActivity::render(RenderLock&&) {
  ...
  for (int i = startIdx; i < endIdx; ++i) {
    ...
    const int rowY =
        PanelDrawHelper::getSeparatorY(layout) + PanelDrawHelper::kPadY + (rowIndex * PanelDrawHelper::kRowH);
