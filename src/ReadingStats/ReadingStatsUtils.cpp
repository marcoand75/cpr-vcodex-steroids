...
bool getCurrentLocalReadingStatsDateTime(ReadingStatsDateTime& outDateTime) {
  // Steroids keeps local time in the global TZ (TimeUtils::configureTimezone)
  // and derives it from the system clock via localtime_r, matching the other
  // reading-stats / manual-date paths. Fall back to the RTC's UTC read when the
  // system clock is not yet valid so per-day time buckets degrade gracefully
