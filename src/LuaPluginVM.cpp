...
static void* luaAlloc(void* ud, void* ptr, size_t osize, size_t nsize) {
  ...
  if (nsize > osize) {
    ...
    if (s_luaAllocated + growth > PLUGIN_MEM_CAP) {
      LOG_INF(TAG, "Memory cap reached: allocated=%u cap=%u growth=%u", s_luaAllocated,
              static_cast<uint32_t>(PLUGIN_MEM_CAP), growth);
...
static void captureError(lua_State* L, const char* prefix) {
  ...
  while (line != nullptr && *line != '\0') {
    ...
    char buf[256];
    const size_t n =
        (static_cast<size_t>(nl - line) < sizeof(buf) - 1) ? static_cast<size_t>(nl - line) : sizeof(buf) - 1;
    memcpy(buf, line, n);
    buf[n] = '\0';
    LOG_ERR(TAG, "  %s", buf);
    line = nl + 1;
