...
bool readFromCache(const std::string& cacheDir, std::string& outTitle, std::string& outAuthor) {
  ...
  outTitle.resize(len);
  if (file.read(reinterpret_cast<uint8_t*>(&outTitle[0]), len) != static_cast<int>(len)) {
    file.close();
    return false;
  }
  ...
  if (file.read(reinterpret_cast<uint8_t*>(&outAuthor[0]), len) != static_cast<int>(len)) {
    file.close();
    return false;
  }
...
bool extractMetadata(const std::string& epubPath, const std::string& cacheDir, std::string& outTitle,
                     std::string& outAuthor, std::string* outSeries, float* outSeriesIndex) {
  outTitle.clear();
  outAuthor.clear();
