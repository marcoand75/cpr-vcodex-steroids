...
bool scan(GfxRenderer& renderer, const Rect& popupRect, const char* rootDir = "/", int* outAdded = nullptr,
          int* outRemoved = nullptr);
...
int queryPage(BookRef* out, int page, int pageSize, SortMode sortMode, const char* searchFilter = nullptr,
              FilterMode filterMode = FilterMode::ALL, int coverWidth = 0, int coverHeight = 0);

// Mixed view: standalone books + series tiles together
int queryMixed(BookRef* out, int page, int pageSize, const char* searchFilter = nullptr,
               FilterMode filterMode = FilterMode::ALL, int coverWidth = 0, int coverHeight = 0,
               SortMode sortMode = SortMode::TITLE_ASC);
int totalMixed();
