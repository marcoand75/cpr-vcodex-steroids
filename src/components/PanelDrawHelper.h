...
class PanelDrawHelper {
 public:
  ...
  static void drawBackground(GfxRenderer& renderer, const PanelLayout& layout) {
    // NOTE: the MarcoAnd75 cyberpunk variant lands with the Lyra MarcoAnd75
    // theme port; until then the default rounded panel is used.
  ...
  static void drawSeparator(GfxRenderer& renderer, const PanelLayout& layout) {
    ...
    // NOTE: the MarcoAnd75 cyberpunk separator lands with the Lyra MarcoAnd75
    // theme port; until then the default rule is used.
    renderer.drawLine(layout.x + kPadX, sepY, layout.x + layout.width - kPadX, sepY, kSeparatorH, true);
  }

  static int getSeparatorY(const PanelLayout& layout) { return layout.y + kPadY + kTitleH + kSeparatorH; }
  ...
  static int getRowTextX(const PanelLayout& layout) { return layout.x + kPadX + kIconPad; }
