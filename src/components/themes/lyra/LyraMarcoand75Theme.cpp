...
void drawCoverPlaceholder(GfxRenderer& renderer, int x, int y, int maxW, int maxH, const char* title) {
  ...
  if (title && title[0]) {
    ...
    renderer.drawText(UI_10_FONT_ID, x + (maxW - titleW) / 2, y + maxH / 3 + 4, truncTitle.c_str(), false,
                      EpdFontFamily::BOLD);
  }
}
...
uint8_t getBookProgress(const RecentBook& b) { return READING_STATS.getBookProgressForHome(b.bookId, b.path); }
...
void fmtDuration(uint64_t ms, char* buf, size_t bufSize) {
  ...
  m %= 60;
  if (h)
    snprintf(buf, bufSize, "%lluh%llum", h, m);
  else
    snprintf(buf, bufSize, "%llum", m);
}
...
void drawDataPanel(const GfxRenderer& r, const RecentBook& book, bool inCar, int px, int py, int pw) {
...

  {
    ...
    // Trend symbol vs daily average
    {
      ...
      const TrendSymbol trend =
          getTrendSymbol(READING_STATS.getTodayReadingMs(), READING_STATS.getGlobalSummary().dailyAverageMs);
...

  {
    ...
    if (!done) {
      ...
      r.drawText(UI_12_FONT_ID, px + pw / 2 - 35, curY + (h3 - r.getLineHeight(UI_12_FONT_ID)) / 2, "COMPLETED", true,
                 EpdFontFamily::BOLD);
    }
    curY += h3 + gap;
  }
...
void LyraMarcoand75Theme::drawRecentBookCover(GfxRenderer& renderer, Rect rect,
  ...
  if (centerIdx != marcoand75LastSelectorIndex) {
    coverRendered = false;
  ...
  const int farRightX = std::min(screenW - kFiveCoverFarW - 8, nearRightX + kFiveCoverNearW - kFiveCoverFarOverlap);
  ...
  auto drawStackedCover = [&](int bookIdx, bool isLeft, bool isFar) -> bool {
    ...
    const int sx = isFar ? (isLeft ? clampedFarLeftX : clampedFarRightX) : (isLeft ? nearLeftX : nearRightX);
    const int sy = isFar ? (isLeft ? farLeftY : farRightY) : (isLeft ? nearLeftY : nearRightY);

    // Aggiunge un bordo bianco (padding) sotto le cover near, esattamente come per la cover centrale
    if (!isFar) {
      renderer.fillRect(sx - kCenterOutlineW, sy - kCenterOutlineW, sw + 2 * kCenterOutlineW, sh + 2 * kCenterOutlineW,
                        false);
    } else {
      ...
      if (Storage.exists(thumbPath.c_str())) {
        ...
        if (Storage.openFileForRead("HOME", thumbPath, file)) {
          ...
          if (bitmap.parseHeaders() == BmpReaderError::Ok) {
            bool drawn = false;
            const std::string rawPath = CoverRawCache::getRawPath(thumbPath, bitmap.getWidth(), bitmap.getHeight());
            RawCoverHeader rawHeader;
            size_t rawSize = 0;
            uint8_t* rawData = CoverRawCache::load(rawPath, &rawHeader, &rawSize);
            if (rawData) {
              const float bmpRatio = static_cast<float>(rawHeader.width) / static_cast<float>(rawHeader.height);
              ...
              if (bmpRatio > tileRatio) {
                ...
                int drawY = sy;
                LOG_DBG("HCR-RAW", "side cover RAW bmp=%dx%d tile=%dx%d draw=%dx%d@%d,%d rawPath=%s", rawHeader.width,
                        rawHeader.height, sw, sh, drawW, drawH, drawX, drawY, rawPath.c_str());
                renderer.drawBitmapFromRaw(rawHeader.width, rawHeader.height, rawHeader.topDown, rawHeader.rowBytes,
                                           rawData, drawX, drawY, drawW, drawH, 0.0f, 0.0f);
                ...
                renderer.drawBitmapFromRaw(rawHeader.width, rawHeader.height, rawHeader.topDown, rawHeader.rowBytes,
                                           rawData, sx, sy, sw, sh, cropX, cropY);
              }
              drawn = true;
              free(rawData);
            }
  ...
  if (!coverRendered) {
    ...
    renderer.fillRect(centerX - kCenterOutlineW - kFiveCoverHaloW, centerCoverTop - kFiveCoverHaloW,
                      kFiveCoverCenterW + 2 * (kCenterOutlineW + kFiveCoverHaloW),
                      kFiveCoverCenterH + 2 * kFiveCoverHaloW, false);
    renderer.fillRect(centerX - kCenterOutlineW, centerCoverTop, kFiveCoverCenterW + 2 * kCenterOutlineW,
...

    {
      ...
      if (!book.coverBmpPath.empty()) {
        thumbPath = UITheme::getCoverThumbPath(book.coverBmpPath, kFiveCoverCenterW, kFiveCoverCenterH);
        if (!Storage.exists(thumbPath.c_str())) {
          thumbPath = UITheme::getCoverThumbPath(book.coverBmpPath, LyraMarcoand75Metrics::values.homeCoverHeight);
        }
        if (!Storage.exists(thumbPath.c_str())) {
          thumbPath = UITheme::getCoverThumbPath(book.coverBmpPath, LyraMarcoand75Theme::kCenterCoverW,
        ...
        if (Storage.openFileForRead("HOME", thumbPath, file)) {
          ...
          if (bitmap.parseHeaders() == BmpReaderError::Ok) {
            ...
            if (rawData) {
              const float bmpRatio = static_cast<float>(rawHeader.width) / static_cast<float>(rawHeader.height);
              const float tileRatio = static_cast<float>(kFiveCoverCenterW) / static_cast<float>(kFiveCoverCenterH);
              const float cropX = (bmpRatio > tileRatio) ? (1.0f - tileRatio / bmpRatio) : 0.0f;
              const float cropY = (bmpRatio < tileRatio) ? (1.0f - bmpRatio / tileRatio) : 0.0f;
              renderer.drawBitmapFromRaw(rawHeader.width, rawHeader.height, rawHeader.topDown, rawHeader.rowBytes,
                                         rawData, centerX, centerCoverTop, kFiveCoverCenterW, kFiveCoverCenterH, cropX,
                                         cropY);
              renderer.maskRoundedRectOutsideCorners(centerX, centerCoverTop, kFiveCoverCenterW, kFiveCoverCenterH,
            ...
            if (!drawn) {
              ...
              const float cropX = (bmpRatio > tileRatio) ? (1.0f - tileRatio / bmpRatio) : 0.0f;
              const float cropY = (bmpRatio < tileRatio) ? (1.0f - bmpRatio / tileRatio) : 0.0f;
              renderer.drawBitmap(bitmap, centerX, centerCoverTop, kFiveCoverCenterW, kFiveCoverCenterH, cropX, cropY);
              renderer.maskRoundedRectOutsideCorners(centerX, centerCoverTop, kFiveCoverCenterW, kFiveCoverCenterH,
                                                     kCornerRadius, Color::White);
              CoverRawCache::generate(thumbPath, rawPath);
            }
      ...
      if (!hasCover) {
        drawCoverPlaceholder(renderer, centerX, centerCoverTop, kFiveCoverCenterW, kFiveCoverCenterH,
                             recentBooks[centerIdx].title.c_str());
      }
      if (hasCover && !book.path.empty()) {
        SummaryJSON::BookBadge badge;
        if (READING_STATS.getBookHomeStats(book.bookId, book.path, badge) && badge.completed) {
          drawReadRibbon(renderer, centerX, centerCoverTop, kFiveCoverCenterW, kFiveCoverCenterH);
        }
      }
    }

    if (inCarouselRow) {
      renderer.drawRoundedRect(centerX - 1, centerCoverTop - 1, kFiveCoverCenterW + 2, kFiveCoverCenterH + 2, 4,
                               kCornerRadius + 1, true);
    ...
    for (int i = 0; i < bookCount; ++i) {
      if (i == centerIdx)
        renderer.fillRect(dotX, dotsY + 12, kDotSize, kDotSize, true);
      else
        renderer.drawRect(dotX, dotsY + 12, kDotSize, kDotSize, true);
...
void LyraMarcoand75Theme::drawCarouselBorder(GfxRenderer& renderer, Rect rect, bool inCarouselRow) const {
  ...
  renderer.drawRoundedRect(centerX, centerTileY + 12, kFiveCoverCenterW, kFiveCoverCenterH, kSelectionLineW,
                           kCornerRadius, true);
}
...
void LyraMarcoand75Theme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
  ...
  for (int slot = 0; slot < visibleCount; ++slot) {
    ...
    if (safeSelectedIndex == i) {
      ...
      renderer.fillRoundedRect(iconX - kHighlightPad32, highlightY, highlightSize, highlightSize, kCornerRadius,
                               Color::Black);
    }
    if (rowIcon != nullptr) {
...
void LyraMarcoand75Theme::drawTabBar(const GfxRenderer& renderer, Rect rect, const std::vector<TabInfo>& tabs,
                                     bool selected) const {
