...
bool exists() { return LibraryIndex::exists(); }
...
bool load(std::vector<Entry>& out) {
  ...
  if (fsz < sizeof(LibraryIndex::Record)) {
    f.close();
    return false;
  }
  ...
  for (int i = 0; i < count; ++i) {
    if (f.read(reinterpret_cast<uint8_t*>(&rec), sizeof(LibraryIndex::Record)) !=
        static_cast<int>(sizeof(LibraryIndex::Record)))
      break;
...
void invalidate() { LibraryIndex::invalidate(); }
...
bool removeBook(const std::string& path) {
  ...
  if (!f) return false;
  LibraryIndex::Record rec;
  while (f.read(reinterpret_cast<uint8_t*>(&rec), sizeof(LibraryIndex::Record)) ==
         static_cast<int>(sizeof(LibraryIndex::Record))) {
...
bool scan(GfxRenderer& renderer, const Rect& popupRect, std::vector<Entry>& out, const char* rootDir, int) {
