...
1. Download the latest release `.bin` from the GitHub Releases page.
2. Copy it to the root of a FAT32 microSD card.
3. Insert the card into the powered-off Xteink X4 (or X3), then hold the physical **OK / Select** button while switching the device on.
4. The bootloader detects `firmware.bin` and flashes it automatically.

For X4 Pro or USB-locked X3 recovery, see the notes in this README and the release workflow.

## Building

- Use PlatformIO with the repo's `platformio.ini`.
- Recommended local builds:
  - `python -X utf8 -m platformio run -e default -j 16`
  - `python -X utf8 -m platformio run -e gh_release -j 16`
- The `gh_release` environment is used for packaging and validation before publishing a release.

## Device support

- Tested on Xteink X4.
- Xteink X3 support is included through runtime board/panel detection and the same firmware image; physical validation on X3, especially UC8279d-based units, remains community-reported rather than personally tested.

## Contributing

Contributions are welcome. Please open an issue or pull request against this repository.

## License

This project is built on top of CrossPoint Reader and CPR-vCodex. See the upstream license terms in the source tree; Steroids-specific additions follow the same open-source approach unless otherwise noted.
